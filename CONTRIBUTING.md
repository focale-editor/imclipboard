# Contributing to Imclipboard

Thanks for helping improve Imclipboard. Bug reports, focused fixes, documentation improvements, and well-scoped features are welcome.
...
- Search [existing issues](https://github.com/focale-editor/imclipboard/issues) before opening a new one.
...
Imclipboard follows the SDK constraint in [`pubspec.yaml`](pubspec.yaml). Install a compatible stable Flutter SDK, fork the repository, and run:

```sh
git clone https://github.com/YOUR_ACCOUNT/imclipboard.git
cd imclipboard
flutter pub get
```
...
When changing platform code, exercise the affected platform and the example application when possible.
